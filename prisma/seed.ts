...
const products = [
  ...
  },
  {
    name: "Yoga Mat",
    slug: "yoga-mat",
    description: "Non-slip yoga mat for home and studio",
    price: 39.99,
    category: "Sports",
    stock: 40,
  ...
  try {
    ...
    for (const product of products) {
      await db.product.create({
        data: { ...product, price: parseFloat(product.price.toString()) },
      });
    }
    ...
    if (!existingSettings) {
      await db.storeSettings.create({
        data: {
          ...
          taxRate: 0.1,
          shippingCost: 10.0,
          enabledCountries: ["United States", "Canada", "United Kingdom", "Australia"],
        },
      });
